...
void CircuitCore::solve()
{
	...
	std::cout << std::to_string(leftoverElement->_resistance).c_str() <<std::endl;
	CircuitCore::getInstance().EquivalentResistance = leftoverElement->_resistance;
	CircuitCore::getInstance().FinalCurrent = leftoverElement->_voltage / leftoverElement->_resistance;
	;
	CircuitCore::getInstance().FinalVoltage = leftoverElement->_voltage 
	;
